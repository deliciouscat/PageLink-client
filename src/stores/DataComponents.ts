...
export const useFileSystemStore = defineStore('fileSystem', () => {
    ...
    // Actions - Item 관리
    function addDocument(collectionId: string, passage: string, tags: string[] = []): Document | null {
    ...
    function generateSampleData(): void {
        ...
        // 뉴스 컬렉션에 아이템 추가
        addDocument(newsCollection.id, 'Vue 3.4 버전이 릴리즈되었습니다. 성능이 크게 개선되었습니다.', ['vue', 'frontend', 'news'])
        addDocument(newsCollection.id, 'TypeScript 5.0의 새로운 기능들을 소개합니다.', ['typescript', 'programming'])
        addDocument(newsCollection.id, 'React 19 베타 버전 출시 소식', ['react', 'frontend', 'news'])

        // 기술 문서 컬렉션
        addDocument(techCollection.id, 'Composite Pattern은 트리 구조를 표현하는 디자인 패턴입니다.', ['design-pattern', 'programming'])
        addDocument(techCollection.id, 'Pinia는 Vue 3를 위한 상태 관리 라이브러리입니다.', ['vue', 'state-management'])
...
export type { FileSystem, LeafNode }